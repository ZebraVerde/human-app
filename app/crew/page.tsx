...
export default function CrewPage() {
  ...
  // Deep links to Instagram Stories camera; link copied to clipboard so user pastes as link sticker
  ...
  // Deep links to Facebook Stories camera; text copied to clipboard; fallback opens share dialog
  async function shareFacebook(e: React.MouseEvent) {
    e.preventDefault()
    if (loading) return
    ...
    const onBlur = () => { opened = true }
    window.addEventListener('blur', onBlur, { once: true })
    window.location.href = 'fb://story/create'
    setTimeout(() => {
      window.removeEventListener('blur', onBlur)
      if (!opened) {
  ...
  const socials = [
    ...
    },
    {
      ...
      // Deep links to Stories camera — link in clipboard, user pastes as link sticker
    ...
    },
    {
      ...
      href: '#', onClick: shareFacebook,
      // Deep links to Facebook Stories camera — text in clipboard; fallback opens share dialog
    ...
    },
    {
      ...
      // share-offsite opens the LinkedIn compose dialog with the URL pre-loaded
